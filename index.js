...
const app = express();
app.use(express.static("../bandhan_eas_client/build"));
app.use(cors());
...
client.connect((err) => {
  const userCollection = client.db("eas_bandhan").collection("users");
  const adminCollection = client.db("eas_bandhan").collection("admins");
  const leadsCollection = client.db("eas_bandhan").collection("leads");
  ...
  app.get("*", (req, res) => {
    res.sendFile(
      path.join(__dirname, "../bandhan_eas_client/build", "index.html")
    );
  });
});
